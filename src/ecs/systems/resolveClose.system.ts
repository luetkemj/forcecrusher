...
export const createResolveCloseSystem = ({ world }: IGameWorld) => {
  const resolveCloseQuery = world.with("tryClose");

  return function resolveCloseSystem() {
    for (const actor of resolveCloseQuery) {
